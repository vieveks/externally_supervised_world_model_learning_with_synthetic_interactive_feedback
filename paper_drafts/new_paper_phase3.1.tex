...
This question leads to a paradox that motivates our investigation. Reinforcement learning can achieve the \textit{same task accuracy} as supervised likelihood training on language-like prediction tasks, yet produces \textit{radically different output distributions}---collapsing uncertainty despite matching structural correctness. Understanding this discrepancy---why two methods that appear equivalent on accuracy metrics diverge so dramatically on distributional metrics---is the central goal of this paper.
...
\begin{enumerate}
    ...
    \item \textbf{Representational similarity} (Section \ref{sec:representations}): Despite behavioral divergence, RL and MLE learn similar internal representations. CKA similarity is 0.66--0.85 overall, and \textit{highest} (0.83) on ambiguous tokens where behavior differs most. This is the representation paradox: maximal representational alignment occurs exactly where behavioral divergence is greatest.
...
Figure \ref{fig:architecture} previews the central mechanism: although RL and MLE learn highly aligned internal representations, their objectives induce fundamentally different output geometries---mode-seeking versus expectation-matching.
...
Policy gradient methods are known to collapse to deterministic policies \cite{williams1992simple}. The objective $\mathbb{E}[\sum r_t]$ has no term encouraging diversity---once a valid action is found, the policy sharpens around it. This mode-seeking behavior is well-documented in maximum entropy RL literature \cite{ziebart2008maxent}, where entropy regularization is explicitly added to prevent collapse.

For prediction tasks with \textit{unique} correct answers, this is fine. But language has ambiguity: multiple continuations may be equally valid (``I went to the [bank/store/park]...''). A collapsed policy cannot represent this uncertainty. Expectation-based theories of language processing \cite{levy2008expectation} suggest that humans maintain probabilistic expectations over continuations---precisely what RL fails to learn.
...
CKA is invariant to orthogonal transformations and isotropic scaling, making it suitable for comparing representations across different training runs. Recent mechanistic interpretability work \cite{elhage2022superposition} has used similar techniques to understand neural network internals; we apply these methods to compare RL and MLE representations.

\subsection{Positioning: Diagnosis, Not Prevention}

Unlike prior work on entropy-regularized or maximum-entropy RL, our goal is not to prevent collapse, but to \textit{characterize} it. We show that distributional collapse is not an optimization failure but the correct solution to the policy gradient objective under ambiguity. This distinguishes our work from approaches that treat entropy loss as a pathology to be mitigated---we demonstrate it is the inevitable consequence of reward-only supervision.
...
\textbf{Key finding: RL collapses to deterministic outputs despite identical accuracy.}

Both methods achieve oracle-level accuracy (80\%), but their \textit{distributions} are fundamentally different. Despite matching on the task metric (accuracy), the entropy differs by a factor of 77$\times$ (0.009 vs. 0.68):
...
The highest CKA (0.83--0.85) occurs for ambiguous tokens---exactly where \textit{behavior} diverges most. This is the representation paradox: maximal representational alignment occurs precisely where behavioral divergence is greatest (entropy differs by 77$\times$, but CKA is 0.83).
...
\subsection{Why Accuracy Masks Distributional Failure}
...
\textbf{Accuracy treats all valid outputs as equivalent.} If the oracle distribution is [0.5, 0.5] over tokens A and B, then predicting A with 100\% confidence achieves 50\% accuracy---the same as predicting B with 100\% confidence. Accuracy cannot distinguish between these two collapsed policies, nor between them and the correct 50/50 distribution.

\textbf{Language requires probability mass allocation.} Downstream tasks depend on calibrated uncertainty:
\begin{itemize}
    \item \textbf{Sampling:} Generation quality depends on the full distribution, not just the mode
    \item \textbf{Perplexity:} The standard evaluation metric directly measures distributional fit
    \item \textbf{Beam search:} Relies on relative probabilities across candidates
    \item \textbf{Uncertainty quantification:} Applications requiring ``I don't know'' responses need calibrated entropy
\end{itemize}

\textbf{RL optimizes correctness, not uncertainty.} The policy gradient objective $\nabla \mathbb{E}[r]$ rewards finding \textit{a} correct answer. Once found, the gradient pushes probability mass toward that answer. There is no term in the objective that rewards spreading mass across multiple valid answers.
...
\textbf{Key Claim:} Likelihood-based training is not required to learn linguistic structure, but \textit{is} required to learn calibrated uncertainty. Reinforcement learning optimizes for correctness, not probability mass allocation. The structural representations are equivalent---only the output distributions differ.
}}
\vspace{0.5em}

\subsection{Contrast with Modern Practice: Pretraining + RLHF vs. RL-Only}

Our findings directly explain why modern LLM pipelines use pretraining followed by RLHF, rather than RL alone. Table \ref{tab:modern_practice} makes this contrast explicit.

\begin{table}[htbp]
\caption{Modern Practice vs. RL-Only Language Learning}
\label{tab:modern_practice}
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Property} & \textbf{Pretraining + RLHF} & \textbf{RL-Only} \\
\midrule
Structure learning & \checkmark & \checkmark \\
Uncertainty learning & \checkmark (from pretraining) & $\times$ (collapses) \\
...
The pretraining phase learns calibrated distributions; the RLHF phase refines behavior while the KL penalty \textit{prevents} distributional collapse. Remove either component, and the pipeline fails:
\begin{itemize}
    \item Without pretraining: No calibrated uncertainty to preserve
    \item Without KL penalty: RL collapses the learned distributions
\end{itemize}
...
This suggests that both methods learn the same ``what is possible'' representation. The difference is entirely in ``how likely is each possibility''---a property of the output layer, not the learned features. Figure \ref{fig:architecture} illustrates this objective-induced divergence.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/objective_divergence.png}
\caption{Objective-induced divergence between representations and output distributions. Despite strong representational alignment (CKA $\approx$ 0.66--0.85), MLE and RL objectives induce fundamentally different output geometries: MLE minimizes KL(data $\|$ model), producing expectation-matching distributions; RL maximizes expected reward, producing mode-seeking collapse. The objective determines distributional geometry---not the learned representations.}
...
\vspace{0.5em}
\noindent\fbox{\parbox{0.95\columnwidth}{
\textbf{Design Implication:} Any RL-based language learner must include either:
\begin{enumerate}
    \item[(a)] Likelihood supervision (pretraining or auxiliary loss), or
    \item[(b)] An explicit distribution-matching regularizer (e.g., KL penalty)
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Algorithms:} Only tested REINFORCE; other policy gradient methods (PPO, SAC) may differ
    \item \textbf{Entropy bonus:} Standard 0.01 coefficient; aggressive entropy regularization might prevent collapse but would fundamentally alter the learning objective
\end{enumerate}

\subsection{Future Work}

\begin{enumerate}
    \item \textbf{Entropy-regularized RL:} Can aggressive entropy bonuses prevent collapse while maintaining task performance?
    \item \textbf{Distributional RL:} Methods that explicitly model return distributions \cite{bellemare2017distributional}
...
\bibitem{ziebart2008maxent} B. D. Ziebart et al., ``Maximum Entropy Inverse Reinforcement Learning,'' in \textit{AAAI}, 2008.

\bibitem{levy2008expectation} R. Levy, ``Expectation-based syntactic comprehension,'' \textit{Cognition}, vol. 106, no. 3, pp. 1126--1177, 2008.
...
\bibitem{elhage2022superposition} N. Elhage et al., ``Toy Models of Superposition,'' \textit{Anthropic}, 2022.

\bibitem{bellemare2017distributional} M. G. Bellemare et al., ``A Distributional Perspective on Reinforcement Learning,'' in \textit{ICML}, 2017.
